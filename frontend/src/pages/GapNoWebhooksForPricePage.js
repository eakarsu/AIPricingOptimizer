...
// Gap (Non-AI): No webhooks for price
// No webhooks for price-change events
import React, { useState } from 'react';

export default function GapNoWebhooksForPricePage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-webhooks-for-price/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>No webhooks for price</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>No webhooks for price-change events</p>
