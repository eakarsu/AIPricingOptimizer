...
export default function GapNoCustomerPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-customer/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>No '/customer</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>No /customer-segment-pricing-ai (per-segment dynamic pricing)</p>
