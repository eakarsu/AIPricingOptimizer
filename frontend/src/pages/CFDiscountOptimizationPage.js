...
// Custom Feature: Discount optimization
// Discount optimization: Customer demographics + purchase history; AI identifies who to offer discounts to (maximize margin + retention)
import React, { useState } from 'react';

export default function CFDiscountOptimizationPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/cf-discount-optimization/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Discount optimization</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Discount optimization: Customer demographics + purchase history; AI identifies who to offer discounts to (maximize margin + retention)</p>
