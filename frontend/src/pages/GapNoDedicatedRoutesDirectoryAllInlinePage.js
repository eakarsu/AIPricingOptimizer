...
// Gap (Non-AI): No dedicated routes directory (all inline
// No dedicated routes directory (all inline — refactor needed)
import React, { useState } from 'react';

export default function GapNoDedicatedRoutesDirectoryAllInlinePage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-dedicated-routes-directory-all-inline/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>No dedicated routes directory (all inline</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>No dedicated routes directory (all inline — refactor needed)</p>
