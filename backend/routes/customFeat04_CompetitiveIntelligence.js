...
// Monitor competitor pricing; AI detects pricing wars; suggests win-back strategies or price matching
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPricingOptimizer / cf-competitive-intelligence',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_competitive_intelligence`;
(async () => {
  try {
    await pool.query(`
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant helping with the feature: 'Competitive intelligence'.
Context: Monitor competitor pricing; AI detects pricing wars; suggests win-back strategies or price matching
...
If input is insufficient, populate sensible defaults and note assumptions in summary.`;
    ...
    console.error('cf-competitive-intelligence/run error:', err);
