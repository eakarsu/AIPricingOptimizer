...
// Gap Feature: No webhooks for price
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPricingOptimizer / gap-no-webhooks-for-price',
    },
    body: JSON.stringify({
...
router.post('/run', async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'No webhooks for price'.\nProject: AIPricingOptimizer.\nContext: No webhooks for price-change events\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['no-webhooks-for-price', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'no-webhooks-for-price', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
