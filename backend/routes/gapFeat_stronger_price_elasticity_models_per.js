...
// Gap Feature: Stronger price elasticity models per
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPricingOptimizer / gap-stronger-price-elasticity-models-per',
    },
    body: JSON.stringify({
...
router.post('/run', async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Stronger price elasticity models per'.\nProject: AIPricingOptimizer.\nContext: Stronger price elasticity models per-product (current implementation is generic)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['stronger-price-elasticity-models-per', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'stronger-price-elasticity-models-per', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    ...
    const r = await _pool.query(
      ...
      ['stronger-price-elasticity-models-per']
    );
