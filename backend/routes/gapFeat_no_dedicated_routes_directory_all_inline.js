...
// Gap Feature: No dedicated routes directory (all inline
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPricingOptimizer / gap-no-dedicated-routes-directory-all-inline',
    },
    body: JSON.stringify({
...
router.post('/run', async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'No dedicated routes directory (all inline'.\nProject: AIPricingOptimizer.\nContext: No dedicated routes directory (all inline — refactor needed)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['no-dedicated-routes-directory-all-inline', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'no-dedicated-routes-directory-all-inline', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    ...
    const r = await _pool.query(
      ...
      ['no-dedicated-routes-directory-all-inline']
    );
