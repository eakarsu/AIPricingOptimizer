...
// Multiple ML models (time series, causal, NLP sentiment); ensemble predicts demand; AI adjusts prices to maximize revenue
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPricingOptimizer / cf-demand-forecasting-ensemble',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_demand_forecasting_ensemble`;
(async () => {
  try {
    await pool.query(`
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant helping with the feature: 'Demand forecasting ensemble'.
Context: Multiple ML models (time series, causal, NLP sentiment); ensemble predicts demand; AI adjusts prices to maximize revenue
...
If input is insufficient, populate sensible defaults and note assumptions in summary.`;
    ...
    console.error('cf-demand-forecasting-ensemble/run error:', err);
