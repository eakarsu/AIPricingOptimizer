...
// Gap Feature: Limited integrations (no Shopify/Amazon/eBay adapters visible)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPricingOptimizer / gap-limited-integrations-no-shopify-amazon-ebay-adapte',
    },
    body: JSON.stringify({
...
router.post('/run', async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Limited integrations (no Shopify/Amazon/eBay adapters visible)'.\nProject: AIPricingOptimizer.\nContext: Limited integrations (no Shopify/Amazon/eBay adapters visible)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['limited-integrations-no-shopify-amazon-ebay-adapte', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'limited-integrations-no-shopify-amazon-ebay-adapte', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    ...
    const r = await _pool.query(
      ...
      ['limited-integrations-no-shopify-amazon-ebay-adapte']
    );
