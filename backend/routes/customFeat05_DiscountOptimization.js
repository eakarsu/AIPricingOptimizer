...
// Customer demographics + purchase history; AI identifies who to offer discounts to (maximize margin + retention)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPricingOptimizer / cf-discount-optimization',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_discount_optimization`;
(async () => {
  try {
    await pool.query(`
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant helping with the feature: 'Discount optimization'.
Context: Customer demographics + purchase history; AI identifies who to offer discounts to (maximize margin + retention)
...
If input is insufficient, populate sensible defaults and note assumptions in summary.`;
    ...
    console.error('cf-discount-optimization/run error:', err);
